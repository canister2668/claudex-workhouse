<script lang="ts">
  ...
  export let value:WorkMode="default";
  export let disabled=false;
  export let onchange:(mode:WorkMode)=>void=()=>{};
  $: items=[
    {id:"default" as const,label:$t("workMode.default"),description:$t("workMode.defaultDescription")},
    {id:"plan" as const,label:$t("workMode.plan"),description:$t("workMode.planDescription")}
  ];
  let root:HTMLDivElement,menu:HTMLDivElement,open=false,menuStyle="",menuWidth=220;
  $: selected=items.find(item=>item.id===value)??items[0];
  $: if(disabled&&open)close();
  function place(){
    if(!root||!menu)return;
    const band=currentViewportBand(),rect=root.getBoundingClientRect();
    menuWidth=Math.min(220,Math.max(160,band.width-16));
  ...
  function show(){if(disabled)return;menuWidth=Math.min(220,Math.max(160,currentViewportBand().width-16));menuStyle=`width:${menuWidth}px`;open=true;requestAnimationFrame(()=>{try{menu.showPopover();}catch{}place();});}
  function close(){open=false;try{menu?.hidePopover();}catch{}}
  function toggle(){open?close():show();}
  function choose(mode:WorkMode){onchange(mode);close();}
...
<div bind:this={root} class="compact-choice" role="group" aria-label={$t("workMode.label")} data-provider={provider}>
  <button type="button" class="choice-trigger" {disabled} aria-label={selected.label} aria-haspopup="listbox" aria-expanded={open} onclick={toggle}><span>{selected.label}</span>{#if open}<ChevronDown size={15}/>{:else}<ChevronUp size={15}/>{/if}</button>
  <div bind:this={menu} class="choice-menu" popover="manual" role="listbox" aria-label={$t("workMode.label")} style={menuStyle}>
    {#each items as item}
      <button type="button" role="option" aria-selected={value===item.id} onclick={()=>choose(item.id)}><span><strong>{item.label}</strong><small>{item.description}</small></span>{#if value===item.id}<Check size={16}/>{/if}</button>
    {/each}
  </div>
</div>
...
<style>
  ...
  .choice-trigger{min-width:102px;min-height:36px;padding:0 10px;display:flex;align-items:center;justify-content:space-between;gap:7px;border:1px solid var(--line);border-radius:10px;background:var(--surface);color:var(--accent-strong);box-shadow:var(--shadow);font-size:.72rem;font-weight:800;white-space:nowrap}
  ...
  .choice-menu button>span{display:flex;min-width:0;flex-direction:column;gap:2px}.choice-menu strong{font-size:.74rem}.choice-menu small{color:var(--muted);font-size:.64rem;line-height:1.3}
  @media(max-width:600px){.choice-trigger{min-width:96px;min-height:34px;padding-inline:9px}}
</style>
