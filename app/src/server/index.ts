...
import { CollaborationBoardService } from "./collaboration-board.js";
import { CollaborationBoardAutomationEngine } from "./collaboration-board-automation.js";
...
const createBody = promptBody.extend({
  ...
  if (value.provider !== "codex" && value.reasoningEffort && !ClaudeProvider.validEfforts.has(value.reasoningEffort)) context.addIssue({ code:z.ZodIssueCode.custom, message:"Unknown compatible-runtime reasoning effort." });
...
const boardActions={
  ...
  "resume":async(_request:FastifyRequest|null,chainId:string,raw:Record<string,unknown>)=>{
    const linked=(await db.listTasks()).filter(item=>item.workChainId===chainId).sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt)),taskId=z.string().min(1).max(200).parse(raw.taskId??linked[0]?.id),prompt=z.string().trim().min(1).max(config.promptMaxLength).parse(raw.prompt??"Continue this work from the latest persisted state."),task=await db.getTask(taskId);if(!task||task.workChainId!==chainId)throw Object.assign(new Error("Task is not linked to this collaboration board card."),{statusCode:409,code:"BOARD_TASK_MISMATCH"});const next=await withThreadTurn(task.provider,task.threadId,async()=>publishTaskSnapshot(await db.upsertTask(await provider(task.provider).sendMessage(task,prompt))));return{task:next};
  },
...
const boardAutomation=new CollaborationBoardAutomationEngine(collaborationBoard,async(action,chainId,context)=>boardActions[action](null,chainId,{},context));
