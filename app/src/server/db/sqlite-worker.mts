...
function handle(op:string,p:Row):any{
  ...
  if(op==="list_tasks_by_work_chain_ids"){const ids=(Array.isArray(p.chainIds)?p.chainIds:[]).filter((id:unknown):id is string=>typeof id==="string"&&id.length>0).slice(0,200);if(!ids.length)return[];return all(`SELECT * FROM tasks WHERE work_chain_id IN (${ids.map(()=>"?").join(",")}) ORDER BY updated_at DESC`,ids).map(taskRow);}
  ...
  if(op==="list_collaboration_sessions_by_work_chain_ids"){const ids=(Array.isArray(p.chainIds)?p.chainIds:[]).filter((id:unknown):id is string=>typeof id==="string"&&id.length>0).slice(0,200);if(!ids.length)return[];return all(`SELECT * FROM collaboration_sessions WHERE work_chain_id IN (${ids.map(()=>"?").join(",")})${p.includeArchived?"":" AND archived_at IS NULL"} ORDER BY updated_at DESC`,ids).map(row=>objectRow(row,new Set(["current_turn_counts_json","metadata_json"])));}
