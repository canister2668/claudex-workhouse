...
describe("private version policy", () => {
  it("allows versions up to and including 1.0.3 while private", () => {
    expect(PRIVATE_VERSION_MAX).toBe("1.0.3");
    expect(() => assertPrivateVersionPolicy({ private: true, version: "0.1.0" })).not.toThrow();
    expect(() => assertPrivateVersionPolicy({ private: true, version: "1.0.3" })).not.toThrow();
  });

  it("blocks versions above 1.0.3 while private", () => {
    expect(() => assertPrivateVersionPolicy({ private: true, version: "1.0.4" }))
      .toThrow(/must not exceed 1\.0\.3/);
